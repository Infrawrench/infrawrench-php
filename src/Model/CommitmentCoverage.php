<?php

/*
 * infrawrench/sdk v1.27.0 | MIT | Copyright (c) 2026 Infrawrench LLC
 * https://github.com/Infrawrench/Infrawrench
 *
 * Generated from the Infrawrench API OpenAPI 3.1 spec (API version 1.27.0).
 *
 * DO NOT EDIT. Regenerate with:
 *   pnpm --filter @infrawrench/web generate:sdk
 *
 * Internal routes are absent by construction: the generator consumes the same
 * published spec that /openapi.json serves, which drops every operation
 * marked x-internal.
 */

declare(strict_types=1);

namespace Infrawrench\Sdk\Model;

use Infrawrench\Sdk\Internal\Coerce;

final class CommitmentCoverage implements \JsonSerializable
{
    /**
     * @param bool $available False when every in-scope account was excluded — 'we cannot tell' reported as unavailable, never as 0%.
     * @param list<CommitmentCoverageCurrency> $currencies
     * @param list<string> $excludedAccountIds Accounts whose plugin cannot tell usage from other charge types; their rows would drag coverage down for reasons unrelated to purchasing.
     */
    public function __construct(
        public readonly bool $available,
        public readonly array $currencies,
        public readonly array $excludedAccountIds,
    ) {
    }

    /**
     * Build one from a decoded JSON object.
     *
     * @param array<string, mixed> $data
     */
    public static function fromArray(array $data): self
    {
        return new self(
            available: Coerce::toBool($data['available'] ?? null),
            currencies: Coerce::mapList($data['currencies'] ?? null, static fn (mixed $item): CommitmentCoverageCurrency => CommitmentCoverageCurrency::fromArray(Coerce::toArray($item))),
            excludedAccountIds: Coerce::mapList($data['excludedAccountIds'] ?? null, static fn (mixed $item): string => Coerce::toString($item)),
        );
    }

    /**
     * The wire representation, ready for `json_encode`.
     *
     * @return array<string, mixed>
     */
    public function toArray(): array
    {
        return [
            'available' => $this->available,
            'currencies' => array_map(static fn (CommitmentCoverageCurrency $item): array => $item->toArray(), $this->currencies),
            'excludedAccountIds' => $this->excludedAccountIds,
        ];
    }

    /** @return array<string, mixed> */
    public function jsonSerialize(): mixed
    {
        return $this->toArray();
    }
}
