<?php

/*
 * infrawrench/sdk v1.39.0 | MIT | Copyright (c) 2026 Infrawrench LLC
 * https://github.com/Infrawrench/Infrawrench
 *
 * Generated from the Infrawrench API OpenAPI 3.1 spec (API version 1.39.0).
 *
 * DO NOT EDIT. Regenerate with:
 *   pnpm --filter @infrawrench/web generate:sdk
 *
 * Internal routes are absent by construction: the generator consumes the same
 * published spec that /openapi.json serves, which drops every operation
 * marked x-internal.
 */

declare(strict_types=1);

namespace Infrawrench\Sdk\Model;

use Infrawrench\Sdk\Internal\Coerce;

final class PeerPane implements \JsonSerializable
{
    /** @param array<string, mixed> $schema */
    public function __construct(
        public readonly string $tabLabel,
        public readonly string $pluginLogoSvg,
        public readonly string $peerPluginId,
        public readonly array $schema,
    ) {
    }

    /**
     * Build one from a decoded JSON object.
     *
     * @param array<string, mixed> $data
     */
    public static function fromArray(array $data): self
    {
        return new self(
            tabLabel: Coerce::toString($data['tabLabel'] ?? null),
            pluginLogoSvg: Coerce::toString($data['pluginLogoSvg'] ?? null),
            peerPluginId: Coerce::toString($data['peerPluginId'] ?? null),
            schema: Coerce::toArray($data['schema'] ?? null),
        );
    }

    /**
     * The wire representation, ready for `json_encode`.
     *
     * @return array<string, mixed>
     */
    public function toArray(): array
    {
        return [
            'tabLabel' => $this->tabLabel,
            'pluginLogoSvg' => $this->pluginLogoSvg,
            'peerPluginId' => $this->peerPluginId,
            'schema' => $this->schema,
        ];
    }

    /** @return array<string, mixed> */
    public function jsonSerialize(): mixed
    {
        return $this->toArray();
    }
}
